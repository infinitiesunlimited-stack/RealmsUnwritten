...
- Jobs, occupations, labor assignment, and needs — exclusive current-work commitment landed in 0.1E; optional current activity landed in 0.1J; occupation, job markets, schedules, wages, tasks, and needs remain deferred
...
3. **Definition identifiers.** Answered for good types by the Prototype 0.1C architectural review: a durable `FName` authored key is the definition identity, and the dense typed ID is only a runtime handle. Work Types reused the split in 0.1E, Skill Types reused it in 0.1F, and Activity Types reused it in 0.1J. Whether authored keys eventually need versioning rules remains open.

...
- **Extended by:** `SETTLEMENT_PROPERTY_RESIDENCE.md` (Prototype 0.1B), which adds settlements, properties, and household residence; `GOODS_INVENTORY.md` (Prototype 0.1C), which adds good types and inventories; `PHYSICAL_SITE_INVENTORY_LOCATION.md` (Prototype 0.1D), which adds physical sites and inventory location; `WORK_LABOR.md` (Prototype 0.1E), which adds work types and a person's exclusive current-work commitment; `SKILLS_KNOWLEDGE.md` (Prototype 0.1F), which adds Skill Type identity without person skill state; `PERSON_CAPABILITY.md` (Prototype 0.1G–0.1I), which records sparse Person → Skill Type relationships and accumulated practice on the registry, and derives general capability from that practice rather than storing it on the person record; `PERSON_ACTIVITY.md` (Prototype 0.1J), which adds Activity Type identity and one optional current activity on the person; and `TASKS.md` (Prototypes 0.1K and 0.1L), which adds authored Task Type identity and then Task Instance identity, neither carrying any person link. This document is still the record for people, households, and their membership relationship.
...
Nothing else was implemented by this slice. Settlements, properties, and household residence arrived in Prototype 0.1B (`SETTLEMENT_PROPERTY_RESIDENCE.md`), good types and inventories in Prototype 0.1C (`GOODS_INVENTORY.md`), physical sites with inventory location in Prototype 0.1D (`PHYSICAL_SITE_INVENTORY_LOCATION.md`), work types with person current work in Prototype 0.1E (`WORK_LABOR.md`), Skill Type identity in Prototype 0.1F (`SKILLS_KNOWLEDGE.md`), sparse person-capability relationships with accumulated practice and derived general capability in Prototype 0.1G–0.1I (`PERSON_CAPABILITY.md`), Activity Type identity with one optional current activity in Prototype 0.1J (`PERSON_ACTIVITY.md`), Task Type identity in Prototype 0.1K, and Task Instance identity in Prototype 0.1L (`TASKS.md`), where a task holds only its own identifier and its task type, with no lifecycle, participation, or person link. There is still no proficiency labels, knowledge, calendar, occupation, production, commands, events, or save format.
...
- **Occupation** — still deferred. Occupation is a social-economic role and is not current work and not current activity. Prototype 0.1E (`WORK_LABOR.md`) added an exclusive `CurrentWork` commitment on the person, targeted at a physical site; that is labor state, not a profession, and not physical presence. Prototype 0.1J (`PERSON_ACTIVITY.md`) added optional `CurrentActivity`; that is broad recorded behavior, not a profession. Adding a bare occupation string or enum now would still create an unowned field.
...
- **Inventory, possessions, person skill state, kin links, activity, location** — need knowledge, job, and spatial systems that do not exist. Prototype 0.1C added inventories and Prototype 0.1D locates them at physical sites, but a person record still holds no inventory reference: a person is not an inventory holder. Prototype 0.1E added current work as labor commitment, not as person location or presence. Prototype 0.1F added Skill Type identity but no skill field to the person. Prototype 0.1G–0.1I record Person → Skill Type relationships and accumulated practice on the registry and derive general capability from that practice, still without a capability collection on `FPersonRecord`. Prototype 0.1J added one optional `CurrentActivity` on the person as broad recorded behavior, not a task, not presence, and not a capability (`PERSON_ACTIVITY.md`). See `PHYSICAL_SITE_INVENTORY_LOCATION.md`, `WORK_LABOR.md`, `SKILLS_KNOWLEDGE.md`, `PERSON_CAPABILITY.md`, and `PERSON_ACTIVITY.md`.
...
- Jobs, occupations, labor assignment, and needs — exclusive current-work commitment landed in 0.1E; optional current activity landed in 0.1J; authored Task Type identity landed in 0.1K and Task Instance identity in 0.1L; occupation, job markets, schedules, wages, task lifecycle, task participation, and needs remain deferred
- Birth, death, aging, marriage, reproduction, inheritance, household formation and dissolution, migration, and immigration
- Knowledge, teaching, and technology — Skill Type identity landed in 0.1F and the sparse person-capability relationship with accumulated practice and derived general capability landed in 0.1G–0.1I; proficiency labels, knowledge, and teaching remain deferred
...
3. **Definition identifiers.** Answered for good types by the Prototype 0.1C architectural review: a durable `FName` authored key is the definition identity, and the dense typed ID is only a runtime handle. Work Types reused the split in 0.1E, Skill Types reused it in 0.1F, Activity Types reused it in 0.1J, and Task Types reused it in 0.1K. Whether authored keys eventually need versioning rules remains open.
